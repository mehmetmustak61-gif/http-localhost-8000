...
function createMonster() {
  ...
  const shellMat = material(0x1b1d35, { roughness: 0.8, emissive: 0x0b1020, emissiveIntensity: 0.35 });
  const fleshMat = material(0x2d2146, { roughness: 0.82, emissive: 0x170d2f, emissiveIntensity: 0.26 });
  const skullMat = material(0xcbc9d8, { roughness: 0.35, emissive: 0x2b2d3f, emissiveIntensity: 0.2 });
  const clawMat = material(0x0f1424, { roughness: 0.4, metalness: 0.35, emissive: 0x17213e, emissiveIntensity: 0.2 });
  const eyeMat = material(0xffd8b8, { emissive: 0xffb57f, emissiveIntensity: 1.9, roughness: 0.1 });
  const hatMat = material(0x4d2d70, { roughness: 0.7, emissive: 0x2b143f, emissiveIntensity: 0.28 });

  const torso = new THREE.Mesh(new THREE.SphereGeometry(0.72, 18, 14), shellMat);
  torso.position.set(0, 1.45, 0.05);
  group.add(torso);

  const abdomen = new THREE.Mesh(new THREE.SphereGeometry(0.88, 18, 14), fleshMat);
  abdomen.position.set(0, 1.34, -0.92);
  group.add(abdomen);

  const head = new THREE.Mesh(new THREE.SphereGeometry(0.44, 16, 12), skullMat);
  head.position.set(0, 2.02, 0.85);
  group.add(head);

  const mouth = new THREE.Mesh(new THREE.BoxGeometry(0.42, 0.22, 0.28), material(0x1a1c2f, { roughness: 0.6 }));
  mouth.position.set(0, 1.86, 1.13);
  group.add(mouth);

  for (let i = -2; i <= 2; i += 1) {
    const tooth = new THREE.Mesh(new THREE.ConeGeometry(0.03, 0.12, 7), clawMat);
    tooth.position.set(i * 0.06, 1.78, 1.23);
    tooth.rotation.x = Math.PI;
    group.add(tooth);
  }

  const eyeL = new THREE.Mesh(new THREE.SphereGeometry(0.08, 10, 8), eyeMat);
  eyeL.position.set(-0.14, 2.05, 1.16);
  group.add(eyeL);
  const eyeR = new THREE.Mesh(new THREE.SphereGeometry(0.08, 10, 8), eyeMat);
  eyeR.position.set(0.14, 2.05, 1.16);
  group.add(eyeR);

  const neck = new THREE.Mesh(new THREE.CylinderGeometry(0.12, 0.16, 0.5, 10), shellMat);
  neck.position.set(0, 1.74, 0.58);
  neck.rotation.x = Math.PI * 0.46;
  group.add(neck);

  const hatBase = new THREE.Mesh(new THREE.CylinderGeometry(0.08, 0.1, 0.2, 8), hatMat);
  hatBase.position.set(0, 2.34, 0.82);
  group.add(hatBase);
  const hatL = new THREE.Mesh(new THREE.ConeGeometry(0.08, 0.56, 10), hatMat);
  hatL.position.set(-0.12, 2.54, 0.8);
  hatL.rotation.z = 0.45;
  group.add(hatL);
  const hatR = new THREE.Mesh(new THREE.ConeGeometry(0.08, 0.56, 10), hatMat);
  hatR.position.set(0.12, 2.54, 0.8);
  hatR.rotation.z = -0.45;
  group.add(hatR);
  const bellL = new THREE.Mesh(new THREE.SphereGeometry(0.04, 8, 6), eyeMat);
  bellL.position.set(-0.23, 2.73, 0.8);
  group.add(bellL);
  const bellR = new THREE.Mesh(new THREE.SphereGeometry(0.04, 8, 6), eyeMat);
  bellR.position.set(0.23, 2.73, 0.8);
  group.add(bellR);

  const leftArm = new THREE.Mesh(new THREE.CylinderGeometry(0.075, 0.06, 1.8, 10), shellMat);
  leftArm.position.set(-0.68, 1.28, 0.42);
  leftArm.rotation.z = -0.95;
  leftArm.rotation.x = 0.48;
  group.add(leftArm);
  const rightArm = new THREE.Mesh(new THREE.CylinderGeometry(0.075, 0.06, 1.8, 10), shellMat);
  rightArm.position.set(0.68, 1.28, 0.42);
  rightArm.rotation.z = 0.95;
  rightArm.rotation.x = 0.48;
  group.add(rightArm);

  for (const arm of [leftArm, rightArm]) {
    const clawA = new THREE.Mesh(new THREE.ConeGeometry(0.05, 0.24, 6), clawMat);
    clawA.position.set(-0.02, -0.92, 0.04);
    clawA.rotation.x = -Math.PI * 0.3;
    arm.add(clawA);
    const clawB = new THREE.Mesh(new THREE.ConeGeometry(0.05, 0.24, 6), clawMat);
    clawB.position.set(0.02, -0.92, 0.04);
    clawB.rotation.x = -Math.PI * 0.3;
    arm.add(clawB);
  }

  const legL = new THREE.Mesh(new THREE.CylinderGeometry(0.08, 0.06, 1.7, 10), fleshMat);
  legL.position.set(-0.56, 1.08, -0.68);
  legL.rotation.z = -1.12;
  legL.rotation.x = -0.55;
  group.add(legL);
  const legR = new THREE.Mesh(new THREE.CylinderGeometry(0.08, 0.06, 1.7, 10), fleshMat);
  legR.position.set(0.56, 1.08, -0.68);
  legR.rotation.z = 1.12;
  legR.rotation.x = -0.55;
  group.add(legR);

  const extraLegs = [];
  const legZ = [-1.1, -0.6, -0.1];
  for (let i = 0; i < legZ.length; i += 1) {
    const z = legZ[i];
    for (const side of [-1, 1]) {
      const leg = new THREE.Mesh(new THREE.CylinderGeometry(0.07, 0.05, 1.55, 10), fleshMat);
      leg.position.set(side * (0.46 + i * 0.08), 1.04 - i * 0.02, z);
      leg.rotation.z = side * (1.05 + i * 0.18);
      leg.rotation.x = -0.56 + i * 0.12;
      leg.userData.side = side;
      leg.userData.baseY = leg.position.y;
      leg.userData.baseZ = z;
      group.add(leg);

      const tip = new THREE.Mesh(new THREE.ConeGeometry(0.05, 0.24, 7), clawMat);
  ...
  const chestCore = new THREE.Mesh(new THREE.SphereGeometry(0.12, 10, 8), eyeMat);
  chestCore.position.set(0, 1.52, 0.42);
  group.add(chestCore);

  const glow = new THREE.PointLight(0xff365f, 0.9, 10, 2);
  glow.position.set(0, 1.72, 0.32);
  group.add(glow);
  return {
    group,
    pathIndex: 0,
...
function updateEnemy(delta) {
  ...
  monster.group.position.y = 0.04 + Math.sin(now * 4.2 + monster.bob) * 0.05;
  monster.group.scale.setScalar(1 + chaseWeight * 0.05 + Math.sin(now * 2.6) * 0.01);
  monster.group.rotation.z = Math.sin(now * 2.8) * 0.035 * (0.5 + chaseWeight);
  monster.torso.position.y = 1.45 + Math.sin(now * 5.2 + monster.bob) * 0.06;
  monster.torso.rotation.x = -0.06 - lean * 0.45 + Math.sin(now * 4.6) * 0.05;
  monster.torso.rotation.z = Math.sin(now * 4.2) * 0.04;
  monster.head.position.y = 2.02 + Math.sin(now * 8.6) * 0.05;
  monster.head.position.z = 0.85 + Math.sin(now * 7.2) * 0.03;
  monster.head.rotation.x = -0.1 + Math.sin(now * 7.8) * 0.08 - lean * 0.25;
  monster.head.rotation.y = Math.sin(now * 3.5) * 0.16;
  monster.head.rotation.z = Math.sin(now * 8.4) * 0.06 * chaseWeight;
  monster.mouth.scale.y = 1 + Math.max(0, Math.sin(now * 13.5 + 1.4)) * (0.3 + chaseWeight * 0.35);
  monster.mouth.position.z = 1.13 + Math.max(0, Math.sin(now * 10.8)) * 0.05;
  ...
  monster.eyeL.position.y = 2.05 + Math.sin(now * 16) * 0.03;
  monster.eyeR.position.y = 2.05 + Math.sin(now * 16 + 0.7) * 0.03;
  monster.leftArm.rotation.x = 0.48 + Math.sin(stride) * (0.32 + chaseWeight * 0.18);
  monster.rightArm.rotation.x = 0.48 - Math.sin(stride + Math.PI) * (0.32 + chaseWeight * 0.18);
  monster.leftArm.rotation.z = -0.95 + sway - chaseWeight * 0.12;
  monster.rightArm.rotation.z = 0.95 - sway + chaseWeight * 0.12;
  monster.leftArm.position.y = 1.28 + Math.sin(stride + 0.5) * (0.06 + chaseWeight * 0.08);
  monster.rightArm.position.y = 1.28 + Math.sin(stride + Math.PI + 0.5) * (0.06 + chaseWeight * 0.08);
  monster.legL.rotation.x = -0.52 + Math.sin(stride + Math.PI) * 0.26;
  monster.legR.rotation.x = -0.52 + Math.sin(stride) * 0.26;
  monster.legL.position.y = 1.08 + Math.max(0, Math.sin(stride + Math.PI)) * 0.05;
  monster.legR.position.y = 1.08 + Math.max(0, Math.sin(stride)) * 0.05;
  if (monster.extraLegs) {
    for (let i = 0; i < monster.extraLegs.length; i += 1) {
      const leg = monster.extraLegs[i];
      const phase = stride * 0.85 + i * 0.5;
      leg.rotation.x = -0.56 + Math.sin(phase) * (0.16 + chaseWeight * 0.12);
      leg.position.y = leg.userData.baseY + Math.max(0, Math.sin(phase + Math.PI / 2)) * 0.045;
      leg.position.z = leg.userData.baseZ + Math.sin(now * 3 + i) * 0.015;
    }
  }
  monster.glow.intensity = 0.68 + chaseWeight * 0.62 + Math.sin(now * 10.5) * 0.1;
  monster.glow.distance = 8.2 + chaseWeight * 2.1;
